...
public class NullPickaxeEntity extends Monster {
    ...
    public NullPickaxeEntity(EntityType<NullPickaxeEntity> entitytype, Level level) {
        ...
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(BuxinModItems.NULL_PICKAXE.get()));
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new RandomStrollGoal(this, 0.4D, 20) {
            protected Vec3 getPosition() {
                ...
                double x = NullPickaxeEntity.this.getX() + (double) ((random.nextFloat() * 2.0F - 1.0F));
                double y = NullPickaxeEntity.this.getY() + (double) ((random.nextFloat() * 2.0F - 1.0F));
                double z = NullPickaxeEntity.this.getZ() + (double) ((random.nextFloat() * 2.0F - 1.0F));
        ...
        this.goalSelector.addGoal(7, new Goal() {
            ...
            public boolean canUse() {
                return NullPickaxeEntity.this.getTarget() != null && !NullPickaxeEntity.this.getMoveControl().hasWanted();
            }

            public boolean canContinueToUse() {
                return NullPickaxeEntity.this.getMoveControl().hasWanted() && NullPickaxeEntity.this.getTarget() != null && NullPickaxeEntity.this.getTarget().isAlive();
            }

            public void start() {
                LivingEntity livingentity = NullPickaxeEntity.this.getTarget();
                Vec3 vec3 = livingentity.getEyePosition(1.0F);

                NullPickaxeEntity.this.moveControl.setWantedPosition(vec3.x, vec3.y, vec3.z, 2.0D);
            }

            public void tick() {
                LivingEntity livingentity = NullPickaxeEntity.this.getTarget();

                if (NullPickaxeEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
                    NullPickaxeEntity.this.doHurtTarget(livingentity);
                } else {
                    double x = NullPickaxeEntity.this.distanceToSqr(livingentity);
    ...
    public void tick() {
        super.tick();
        if (this.getItemBySlot(EquipmentSlot.MAINHAND).getItem() != BuxinModItems.NULL_PICKAXE.get()) {
