...
public class NullShovelEntity extends Monster {
    ...
    public NullShovelEntity(EntityType<NullShovelEntity> entitytype, Level level) {
        ...
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(BuxinModItems.NULL_SHOVEL.get()));
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new RandomStrollGoal(this, 0.4D, 20) {
            protected Vec3 getPosition() {
                ...
                double x = NullShovelEntity.this.getX() + (double) ((random.nextFloat() * 2.0F - 1.0F));
                double y = NullShovelEntity.this.getY() + (double) ((random.nextFloat() * 2.0F - 1.0F));
                double z = NullShovelEntity.this.getZ() + (double) ((random.nextFloat() * 2.0F - 1.0F));
        ...
        this.goalSelector.addGoal(7, new Goal() {
            ...
            public boolean canUse() {
                return NullShovelEntity.this.getTarget() != null && !NullShovelEntity.this.getMoveControl().hasWanted();
            }

            public boolean canContinueToUse() {
                return NullShovelEntity.this.getMoveControl().hasWanted() && NullShovelEntity.this.getTarget() != null && NullShovelEntity.this.getTarget().isAlive();
            }

            public void start() {
                LivingEntity livingentity = NullShovelEntity.this.getTarget();
                Vec3 vec3 = livingentity.getEyePosition(1.0F);

                NullShovelEntity.this.moveControl.setWantedPosition(vec3.x, vec3.y, vec3.z, 2.0D);
            }

            public void tick() {
                LivingEntity livingentity = NullShovelEntity.this.getTarget();

                if (NullShovelEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
                    NullShovelEntity.this.doHurtTarget(livingentity);
                } else {
                    double x = NullShovelEntity.this.distanceToSqr(livingentity);
    ...
    public void tick() {
        super.tick();
        if (this.getItemBySlot(EquipmentSlot.MAINHAND).getItem() != BuxinModItems.NULL_SHOVEL.get()) {
