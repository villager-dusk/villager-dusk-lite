...
public class NullSwordEntity extends Monster {
    ...
    public NullSwordEntity(EntityType<NullSwordEntity> entitytype, Level level) {
        ...
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(BuxinModItems.NULL_SWORD.get()));
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new RandomStrollGoal(this, 0.4D, 20) {
            ...
            protected Vec3 getPosition() {
                ...
                double d0 = NullSwordEntity.this.getX() + (double) ((random.nextFloat() * 2.0F - 1.0F));
                double d1 = NullSwordEntity.this.getY() + (double) ((random.nextFloat() * 2.0F - 1.0F));
                double d2 = NullSwordEntity.this.getZ() + (double) ((random.nextFloat() * 2.0F - 1.0F));
        ...
        this.goalSelector.addGoal(7, new Goal() {
            ...
            public boolean canUse() {
                return NullSwordEntity.this.getTarget() != null && !NullSwordEntity.this.getMoveControl().hasWanted();
            }

            @Override
            public boolean canContinueToUse() {
                return NullSwordEntity.this.getMoveControl().hasWanted() && NullSwordEntity.this.getTarget() != null && NullSwordEntity.this.getTarget().isAlive();
            }

            @Override
            public void start() {
                LivingEntity livingentity = NullSwordEntity.this.getTarget();
                Vec3 vec3 = livingentity.getEyePosition(1.0F);

                NullSwordEntity.this.moveControl.setWantedPosition(vec3.x, vec3.y, vec3.z, 2.0D);
            }
            ...
            public void tick() {
                ...
                if (NullSwordEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
                    NullSwordEntity.this.doHurtTarget(livingentity);
                } else {
                    double d0 = NullSwordEntity.this.distanceToSqr(livingentity);
    ...
    public void tick() {
        super.tick();
        if (this.getItemBySlot(EquipmentSlot.MAINHAND).getItem() != BuxinModItems.NULL_SWORD.get()) {
