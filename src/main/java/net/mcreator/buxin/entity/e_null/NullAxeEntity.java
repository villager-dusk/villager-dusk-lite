...
public class NullAxeEntity extends Monster {
    ...
    public NullAxeEntity(EntityType<NullAxeEntity> entitytype, Level level) {
        ...
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(BuxinModItems.NULL_AXE.get()));
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new RandomStrollGoal(this, 0.4D, 20) {
            ...
            protected Vec3 getPosition() {
                ...
                double x = NullAxeEntity.this.getX() + (double) ((random.nextFloat() * 2.0F - 1.0F));
                double y = NullAxeEntity.this.getY() + (double) ((random.nextFloat() * 2.0F - 1.0F));
                double z = NullAxeEntity.this.getZ() + (double) ((random.nextFloat() * 2.0F - 1.0F));
        ...
        this.goalSelector.addGoal(7, new Goal() {
            ...
            public boolean canUse() {
                return NullAxeEntity.this.getTarget() != null && !NullAxeEntity.this.getMoveControl().hasWanted();
            }

            @Override
            public boolean canContinueToUse() {
                return NullAxeEntity.this.getMoveControl().hasWanted() && NullAxeEntity.this.getTarget() != null && NullAxeEntity.this.getTarget().isAlive();
            }

            @Override
            public void start() {
                LivingEntity livingentity = NullAxeEntity.this.getTarget();
                Vec3 vec3 = livingentity.getEyePosition(1.0F);

                NullAxeEntity.this.moveControl.setWantedPosition(vec3.x, vec3.y, vec3.z, 2.0D);
            }
            ...
            public void tick() {
                ...
                if (NullAxeEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
                    NullAxeEntity.this.doHurtTarget(livingentity);
                } else {
                    double x = NullAxeEntity.this.distanceToSqr(livingentity);
    ...
    public boolean doHurtTarget(Entity pEntity) {
        ...
        DamageSource source = new DamageSource(this.damageSources().mobAttack(attacker).typeHolder());
    ...
    public void tick() {
        super.tick();
        if (this.getItemBySlot(EquipmentSlot.MAINHAND).getItem() != BuxinModItems.NULL_AXE.get()) {
