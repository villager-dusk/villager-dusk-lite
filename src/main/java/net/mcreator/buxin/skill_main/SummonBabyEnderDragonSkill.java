...
import static net.mcreator.buxin.item.Niubi8Item.spawnBabyEnderDragon;

public class SummonBabyEnderDragonSkill extends WeaponInnateSkill {
    private final StaticAnimation[] animations;
    private static final UUID EVENT_UUID = UUID.fromString("7fbc808b-5aaa-4dfb-ad3f-12462f1b290a");

    public SummonBabyEnderDragonSkill(Builder<? extends Skill> builder) {
        super(builder);
    ...
    public void updateContainer(SkillContainer container) {
        ...
        if (!container.getExecuter().isLogicalClient()) {
            ...
            if (((ServerPlayer)playerpatch.getOriginal()).getMainHandItem().getItem() == BuxinModItems.NIUBI_8.get()) {
                ...
                float maxResource = WeaponInnateContainer.getResource();
                WeaponInnateContainer.setResource(Math.min(newResource, maxResource));
            }
        }
    }
    ...
    public void executeOnServer(ServerPlayerPatch executer, FriendlyByteBuf args) {
        ...
        if (PlayerPatch instanceof ServerPlayerPatch ServerPlayerPatch) {
            ...
            Method_114514.play_sound(player.level, x, y, z, "epicfight:entity.enderdragon.dragon_breath");
            VFXTool.addVFXParticle(player.position(), BuxinMod.MODID, "herobrine_portal", player.level());
            BuxinMod.queueServerWork(20, () -> AnimationPlayer.playAnimation(player, BuxinAnimations.SNAKE_BLADE_SKILL));
            Method_114514.entity_use_command(player, "/weather clear");
            spawnBabyEnderDragon(player.getMainHandItem(), player);
        }
    }
}
