...
/*
public class BabyDragonBeamParticleEmitterInfo extends ParticleEmitterInfo {
    public enum ForwardAxis { PLUS_Z, PLUS_Y }

    private WeakReference<BabyEnderDragonEntity> casterRef = new WeakReference<>(null);
    private WeakReference<LivingEntity> targetRef = new WeakReference<>(null);
    private WeakReference<LivingEntity> e_casterRef = new WeakReference<>(null);
    ...
    public BabyDragonBeamParticleEmitterInfo FollowNormalEntity(LivingEntity caster, LivingEntity target, int durationTicks, ForwardAxis axis, float extraRollRad){
        this.e_casterRef = new WeakReference<>(caster);
        this.targetRef = new WeakReference<>(target);
        this.durationTicks = durationTicks;
        this.axis = axis;
        this.roll = extraRollRad;
    ...
    public void spawnInWorld(Level level, Player player) {
        Optional.ofNullable(EffectRegistry.get(this.effek)).ifPresent(eff -> {
            ...
            final BabyEnderDragonEntity caster0 = casterRef.get();
            ...
            em.addPreDrawCallback((Emitter, partial) -> {
                BabyEnderDragonEntity c = casterRef.get();
                ...
                Vec3 to = (t != null && t.isAlive()) ? eyeLerped(t, partial)
                        : (lastTargetPos != null ? lastTargetPos : from.add(fwd));
                lastTargetPos = to;
